...
from review_visualizer.db.models2 import ReviewSentiment
...
def make_graph(df, total_reviews):
    fig = make_subplots(rows=2, cols=1, specs=[[{"type": "xy", "secondary_y": True}], [{'type':'pie'}]], subplot_titles=("Ratings and Sentiment", "Overall Sentiment"))
    fig.add_trace(
        go.Scatter(x=df['Date'], y=df['Rating'], mode='lines', name="Ratings", line_color="purple"),
        secondary_y=False, row=1, col=1)
    fig.add_trace(
        go.Scatter(x=df['Date'], y=df['Sentiment'], mode='lines', name="Sentiment", line_color="green"),
        secondary_y=True, row=1, col=1)
    fig.add_trace(go.Pie(
        labels = ["Negative", "Positive"], 
